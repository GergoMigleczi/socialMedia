...
class Comment extends Model{
    ...
    public function getComments(int $postId): array {
        try{
            ...
            $this->logger->error('Model/Commet->getComments(): ' . $e->getMessage());
            throw $e;
        }
    ...
    public function saveCommentToDatabase(int $postId, int $profileId, string $content, ?int $parentCommentId = null): ?CommentDTO {
        $this->logger->debug("Model/Commet->saveCommentToDatabase(): Saving comment: post_id=$postId, profile_id=$profileId");
        ...
        try{
            ...
            if (!$this->db->execute()) {
                $this->logger->error("Model/Commet->saveCommentToDatabase(): Failed to save comment to database");
                return null;
            }

            // Get the ID of the newly inserted comment
            $commentId = $this->db->getLastInsertId();
            $this->logger->debug("Model/Commet->saveCommentToDatabase(): New comment ID: $commentId");
            ...
            if (!$profileDto) {
                $this->logger->error("Model/Commet->saveCommentToDatabase(): Failed to get profile info for profile ID: $profileId");
                return null;
            }
            ...
            $this->logger->error('Model/Commet->saveCommentToDatabase(): ' . $e->getMessage());
            throw $e;
        }
    }
}
